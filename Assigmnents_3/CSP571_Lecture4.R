...
t <- duplicated(custdata[,2:ncol(custdata)])
sum(t)
...
custdata[custdata$stateCleaned != custdata$state.of.res,c("state.of.res", 'stateCleaned')]
...
ggplot(data=custdata) + # ggplot2 uses the ggplot() function to declare the graph object
...
######## String Handling
# Material made in collaboration with Prof. James Lamb
...
# Check how many lines are returned
length(bsText)

# Count how many characters exist
oldCharCount<- sum(sapply(bsText, nchar))
oldCharCount
...
newCharCount<- sum(sapply(bsText, nchar))
newCharCount

# What is the most common first letter?
oneLine <- bsText[1]
substr(oneLine, start=1, stop=1)

sort(table(as.vector(sapply(bsText, substr, start=1, stop=1))), decreasing=TRUE)

# We can use a tokenizer to clean up the data
# Note the methodologies for tokenizaton vary great
# Read the docs
...
# Remove all of the punctuation, split the text into individual words, 
# and converted everything into lowercase characters. 
words <- tokenize_words(bsText)
bsText[1]
words[1]

# Let's smash all the words together into one big vector. Then let's look at the distribution of words
?Reduce
allWords <- Reduce(c, words)
sort(table(allWords), decreasing = TRUE)[1:50]

# Maybe we want to split the text into sentances instead.
# Tokenization can also help with that!
# First we need to smash together all the text
allText <- Reduce(paste, Reduce(c, bsText))
head(allText)
sentances <- tokenize_sentences(allText)
sentances[[1]][1]
bsText[1:3]
...
# We would have to dig into the actual heuritistics used to figure out. My theory is that
# it uses a model trained on modern English and the Shakespearan English and punctuation is not
# applying well. Luckily, we have a tool we can use to do this analysis ourselves; regular expressions!
# (jump back to slides)

# Before we do that, let's practice some regex in R

# How many lines contain 9 letter words, in any case
regEx = '\\b[a-zA-Z]{9}\\b'
txt <- grep(pattern = regEx, x=bsText, value = TRUE, fixed = FALSE)
length(txt)
length(bsText)
head(txt)

# Let's extract all of those 9 letter words
v <- regexpr(pattern = regEx, text=bsText)
# Above function returns character position of first match. If no match, -1
NineCharWords <- regmatches(x=bsText, v)
sort(table(NineCharWords), decreasing = TRUE)[1:20]
